   ...
   if(mysqli_num_rows($result)>0){
      ...
      <div class='row'>
         ...
         while ($rows=mysqli_fetch_assoc($result)) {
            ...
            <td>{$rows['query']}</td>
            <td>{$rows['image']}</td>
            <td><a name='' id='' class='btn btn-primary mb-1' href='sendquerymail.php?id={$rows['id']}' role='button'>Send Mail</a>
            <a name='' id='' class='btn btn-danger' href='deletequery.php?id={$rows['id']}' role='button'>Delete</a></td>
         </tr>";
         }
